...
function initializeQuoteForm() {
    ...
    quoteForm.addEventListener('submit', function(e) {
        ...
        // Create mailto link
        const subject = `Drone Photography Quote Request - ${data.serviceType || 'Service'}`;
        const mailtoLink = `mailto:djwisamazing123@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(emailBody)}`;
...
function createEmailBody(data) {
    return `
QUOTE REQUEST FOR DRONE PHOTOGRAPHY SERVICES
...
Event/Project Date: ${data.eventDate || 'Not specified'}
Preferred Time: ${data.eventTime || 'Not specified'}
...
ADDITIONAL SERVICES:
${data.rushJob ? '✓ Rush Job (less than 7 days notice)' : '✗ Standard timeline'}
${data.editing ? '✓ Professional editing and color correction needed' : '✗ Basic editing only'}
${data.usb ? '✓ USB drive included with final photos/videos' : '✗ Digital delivery only'}

Please provide a detailed quote for the above requirements. Thank you!
